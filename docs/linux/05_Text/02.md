...
如果遇到网络问题无法使用 `wget` 命令在线安装 vim-plug，可以从 vim-plug 的 GitHub 仓库手动下载 [`plug.vim` 文件](https://github.com/junegunn/vim-plug/blob/master/plug.vim)，并将其放置到  Vim 配置目录中：
...
- `[Plugins] .vimrc`：表示 vim-plug 正在处理的 `~/.vimrc` 文件中列出的插件
- `Finished. 0 error(s).`：表示插件安装或更新过程已经完成，并且没有遇到任何错误
- `[=]`：表示插件安装或更新的进度条，[=] 表示进度已经完成
- `- vim-plug: OK`：表示 vim-plug 本身的状态是 OK，没有错误
