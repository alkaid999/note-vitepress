...
- **补丁文件**：如果需要对源代码进行修改，可以使用 Patch 标签指定补丁文件
...
mv nginx-1.27.2.tar.gz ~/rpmbuild/SOURCES/
```

注意，当在构建 RPM 包时，构建系统会检查机器上是否安装了构建该软件包所需的依赖项，也就是说需要安装 Nginx 相关的依赖：

```bash
dnf -y install gcc make openssl-devel  pcre-devel zlib-devel
```
...
`%prep` 这是一个宏，用于执行准备工作阶段的任务，通常是在实际编译之前。在这个宏中，可以放置任何需要在构建开始之前执行的命令，比如复制补丁文件、设置环境变量等。如果没有特别的准备工作要做，`%prep` 宏可以为空或者不写。
...
```bash
scp ~/rpmbuild/RPMS/x86_64/nginx-1.27.2-1.el9.x86_64.rpm root@192.168.121.129:/root/
```
...
```bash
rpm -qi nginx-1.27.2-1.el9.x86_64.rpm
```
...
```bash
rpm -ivh nginx-1.27.2-1.el9.x86_64.rpm
```
...
::: code-group
```python [~/rpmbuild/SPECS/nginx.spec]
Name:           nginx
Version:        1.27.2
Release:        1%{?dist}
Summary:        A high performance web server and reverse proxy server
