...
- `rocky-addons.repo`：这是附加组件仓库，包含一些更特殊场景的附加组件或扩展包。内容包括：特定硬件的专用驱动、第三方合作的软件包、或 Rocky Linux 团队提供的附加功能组件（不属于核心或 extras 范畴）。这类包通常针对特定需求（如企业级扩展、特殊硬件支持），普通用户可能较少用到
- `rocky-devel.repo`：这是额外补充仓库，包含一些不属于核心系统，但对部分用户有用的额外软件包。例如：一些兼容性工具、小众但实用的工具、非核心的驱动或库等。这些包通常不包含在 rocky.repo 中，但又不属于开发专用或特殊附加组件，属于锦上添花的补充
- `rocky-extras.repo`：这是开发专用仓库，主要提供软件开发相关的工具和库（尤其是编译依赖）。例如：各种编程语言的开发包（如 gcc-devel、python3-devel）、头文件（`.h` 文件）、静态库等。如果需要编译源码、开发软件，通常需要从这个仓库安装依赖。
- `rocky.repo`：这是核心基础仓库，包含 Rocky Linux 操作系统运行必需的核心组件、基础工具和主流应用程序。通常整合了类似 RHEL 中的 BaseOS（基础系统包，如内核、系统工具）和 AppStream（应用程序流，如常用服务、工具软件）。是系统默认启用的核心仓库，提供最稳定、最常用的软件包，满足日常系统运行和基础应用需求
