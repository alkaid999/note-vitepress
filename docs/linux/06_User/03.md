...
1. 创建用户
    ...
    如果没有指定用户组，系统会自动创建一个与用户名相同的新用户组，并将新用户添加到这个组中：
...
2. 创建用户并指定主目录路径
    ...
    默认情况下主目录会在 /home/ 用户名 目录下，可以通过选项 `-md` 进行修改指定的位置：

    ```bash
    useradd -md /opt/customhome/tom tom
    ```
...
4. 创建用户并指定附加用户组
    ...
    ```bash
    useradd -g users -G games,wheel john
    ```
...
6. 创建用户并添加注释信息
    ...
    ```bash
    useradd -c "Accounting Department" eva
    ```
...
`usermod` 命令和 `useradd` 命令有许多相似的选项，因为 `usermod` 命令的主要功能是修改由 `useradd` 命令创建的用户账户信息。
...
3. 创建一个重复 GID 的用户组
    ...
    ```bash
    groupadd -g 6000 existing_group
    ```
    ...
    ```bash
    groupadd -og 6000 existing_group
    ```

    ```bash
    grep -E "existing_group|test_group"  /etc/group
    ```
...
```bash
grep -E '\btemp\b' /etc/passwd /etc/group /etc/gshadow
```
...
```console
Setting group administrator for cloud_group to eva
```

再通过 `/etc/gshadow` 进行确认：

```bash
grep "cloud_group" /etc/gshadow
```
