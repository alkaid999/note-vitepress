...
1. 复制系统命令
    ...
    ```bash
    cp /usr/bin/touch /usr/bin/my_touch
    ```

2. 设置 SUID 权限

    为 `/usr/bin/my_touch` 设置 `SUID` 权限：

    ```bash
    chmod u+s /usr/bin/my_touch
    ```
...
3. 切换到普通用户，尝试创建文件
    ...
    ```shell
    [eva@localhost ~]$ /usr/bin/my_touch  /test_file
    ```
...
```bash
chmod +x /opt/test_locate.sh
```

```bash
ll /opt/test_locate.sh 
```
...
```bash
sudo -u eva touch /tmp/eva_file.txt
```

然后使用 `tom` 尝试删除 `eva` 在 `/tmp` 目录下创建的文件：

```bash
sudo -u tom rm -f /tmp/eva_file.txt 
```
