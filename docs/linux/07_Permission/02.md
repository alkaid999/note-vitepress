...
4. 使用参考文件更改所属组
    ...
    ```bash
    chgrp --reference=access.log anaconda-ks.cfg 
    ```
...
2. 更改文件的所属组
    ...
    ```bash
    chown :dev_team anaconda-ks.cfg
    ```
...
```shell
```bash
chown john:test_group anaconda-ks.cfg
```
...
4. 直接赋予权限：
    ...
    ```bash
    chmod u=rw,g=rw,o=r anaconda-ks.cfg
    ```
