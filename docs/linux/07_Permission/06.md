...
| 对比维度 | su                                         | sudo                                           |
| -------- | ------------------------------------------ | ---------------------------------------------- |
| 核心功能 | 直接切换到目标用户身份（含 root）          | 不切换身份，临时以指定用户权限执行特定命令     |
| 权限范围 | 获得目标用户完整权限（含环境变量、主目录） | 仅获得命令所需的特定权限，权限粒度更细         |
| 安全特性 | 无精细化控制，风险较高                     | 通过 /etc/sudoers 配置，支持用户、命令精准管控 |
| 验证方式 | 需输入目标用户（如 root）的密码            | 需输入当前用户密码（可配置免密）               |
| 适用场景 | 需长期以目标用户身份操作时                 | 临时执行高权限命令，需安全审计与权限管控时     |
